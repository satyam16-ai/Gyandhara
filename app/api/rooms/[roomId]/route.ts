...
export async function GET(
  ...
  try {
    ...
    const backendUrl = `http://localhost:8080/api/rooms/${roomId}`
    ...
    if (!response.ok) {
      return NextResponse.json(
        { success: false, error: data.error || 'Failed to fetch room details' },
    ...
    console.error('Error proxying room details request:', error)
    return NextResponse.json(
